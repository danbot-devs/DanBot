const fs = require('fs');
module.exports = (bot, utils, ytdl, config, message) => {
      const prefix = "!";
    ...
    fs.readdir("./musiccommands/", (err, files) => {
        ...
        jsfiles.forEach((f, i) => {
            let props = require(`../musiccommands/${f}`);
    ...
    bot.loadCommand = (commandName) => {
        try {
            ...
            return utils.cmd_fail(`Error: ${err}\nCommand \`${commandName}\` cannot be found.`, `${prefix}reload <command>`);
        }
    };
    ...
    bot.unloadCommand = async (commandName) => {
        try {
            ...
            delete require.cache[require.resolve(`../musiccommands/${commandName}.js`)];
            return false;
        } catch (err) {
            return utils.cmd_fail(`Error: ${err}\nCommand \`${commandName}\` cannot be found.`, `${prefix}reload <command>`);
        }
